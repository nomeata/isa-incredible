  ...
  next
    ...
    proof (rule eff.intros)
      ...
      proof(induct rule: hyps_alongE)
        ...
        hence "vidx v'' \<noteq> vidx v'" using `v' |\<in>| vertices` `v'' |\<in>| vertices` vidx_inj
          by (meson inj_onD notin_fset)
        hence "freshenV (vidx v') ` a_fresh ant \<inter> freshenV (vidx v'') ` pre_fv (labelsOut (nodeOf v'') h'') = {}"
